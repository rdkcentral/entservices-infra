...
namespace WPEFramework {
    ...
    {
        LOGINFO("naveen ResourceManagerImplementation Constructor");
    ...
    /* virtual */ Core::hresult ResourceManagerImplementation::SetAVBlocked(const string& appId, const bool blocked, string& message, bool& success) /* override */
    {
        ...
        try {
            ...
            if ((nullptr != mEssRMgr) && (false == mDisableBlacklist)) {
                ...
                if (true == status) {
                    ...
                    success = true;
                    // No message set for successful operations (matches original)
                } else {
                    LOGERR("ERM failed to %s application: %s", blocked ? "block" : "unblock", appId.c_str());
                    message = "ERM failed to update application status";
                ...
                // Original logic: return success with message when ERM not available or disabled
                message = (mDisableBlacklist) ? "Blacklist RFC is disabled" : "ERM not enabled";
                success = true;  // Original returns success with message
                result = Core::ERROR_NONE;
            }
    ...
    /* virtual */ Core::hresult ResourceManagerImplementation::ReserveTTSResource(const string& appId, Exchange::IResourceManager::TTSResult& ttsResult) /* override */
    {
        ...
        Core::hresult returnCode = Core::ERROR_NONE;  // Always return success from COMRPC
        bool success = false;
        ...
        try {
            ...
            if (false == mDisableReserveTTS) {
                ...
                if (status) {
                    ...
                    returnCode = Core::ERROR_NONE;  // Always return success from COMRPC
                    success = false;                // Use success parameter to indicate failure
                ...
                returnCode = Core::ERROR_NONE;
                success = false;  // RFC disabled returns false (matches current behavior)
            }
            
        } catch (const std::exception& e) {
            LOGERR("Exception in ReserveTTSResource: %s", e.what());
            returnCode = Core::ERROR_NONE;  // Always return success from COMRPC
            success = false;                // Use success parameter to indicate exception
        }
    ...
    /* virtual */ Core::hresult ResourceManagerImplementation::ReserveTTSResourceForApps(IAppIdIterator* appids, Exchange::IResourceManager::TTSResult& ttsResult) /* override */
    {
        ...
        if (appids == nullptr) {
            ...
            success = false;
            ttsResult.success = success;
            return Core::ERROR_NONE;  // Return success but with success=false
        }
        ...
        try {
            ...
            if (false == mDisableReserveTTS) {
                ...
                LOGWARN("ReserveTTS RFC is disabled");
                returnCode = Core::ERROR_NONE;
                success = false;  // RFC disabled means operation fails
            }
