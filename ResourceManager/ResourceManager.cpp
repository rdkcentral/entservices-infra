...
namespace WPEFramework
{
    ...
    namespace Plugin
    {
        ...
        const string ResourceManager::Initialize(PluginHost::IShell* service)
        {
            ...
            _service = service;
            _service->AddRef();
            _implementation = _service->Root<Exchange::IResourceManager>(_connectionId, 5000, _T("ResourceManagerImplementation"));

            if (nullptr != _implementation)
            {
                // Register JSON-RPC interface
                Exchange::JResourceManager::Register(*this, _implementation);
            }
            else
            {
