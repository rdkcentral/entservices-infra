...
namespace Web {
    ...
    public:
        ...
        typedef HWebSocketLinkType<LINK, INBOUND, OUTBOUND, ALLOCATOR> ParentClass;
...
POP_WARNING()
            ...
            void ReceivedWebSocket(Core::ProxyType<INBOUND>& element, const TemplateIntToType<1>& /* For compile time diffrentiation */)
            {
                ...
                if ((element->Upgrade.IsSet()) && (element->Upgrade.Value() == Request::UPGRADE_WEBSOCKET) && (element->Connection.IsSet()) && (element->Connection.Value() == Request::CONNECTION_UPGRADE)) {
                    ...
                    if ((_state & WEBSERVER) == 0) {
                        ...
                        _path = element->Path;
                        LOGINFO("Is Query set: %d,  %s", element->Query.IsSet(), element->Query.Value().c_str());
    ...
    public:
        HWebSocketLinkType() = delete;
        HWebSocketLinkType(const HWebSocketLinkType<LINK, INBOUND, OUTBOUND, ALLOCATOR>& copy) = delete;
        HWebSocketLinkType<LINK, INBOUND, OUTBOUND, ALLOCATOR>& operator=(const HWebSocketLinkType<LINK, INBOUND, OUTBOUND, ALLOCATOR>& RHS) = delete;

PUSH_WARNING(DISABLE_WARNING_THIS_IN_MEMBER_INITIALIZER_LIST)
        template <typename... Args>
        HWebSocketLinkType(const bool binary, const bool masking, const uint8_t queueSize, Args&&... args)
            : _channel(*this, binary, masking, queueSize, std::forward<Args>(args)...)
        {
        }
        template <typename... Args>
        HWebSocketLinkType(const bool binary, const bool masking, const uint8_t queueSize, ALLOCATOR allocator, Args&&... args)
            : _channel(*this, binary, masking, queueSize, allocator, std::forward<Args>(args)...)
        {
        }
POP_WARNING()
        virtual ~HWebSocketLinkType() = default;
    ...
    public:
        ...
        {
            LOGDBG("Opening Websocket TCP Binding");
    ...
    class HWebSocketServerType {
    private:
        typedef HWebSocketServerType<LINK> ThisClass;

        template <typename ACTUALLINK>
        class Handler : public HWebSocketLinkType<ACTUALLINK, Web::Request, Web::Response, WebSocket::RequestAllocator&> {
        private:
            typedef HWebSocketLinkType<ACTUALLINK, Web::Request, Web::Response, WebSocket::RequestAllocator&> BaseClass;
    ...
    public:
        HWebSocketServerType() = delete;
        HWebSocketServerType(const HWebSocketServerType<LINK>& copy) = delete;
        HWebSocketServerType<LINK>& operator=(const HWebSocketServerType<LINK>& RHS) = delete;

        template <typename... Args>
        HWebSocketServerType(const bool binary, const bool masking, Args&&... args)
            : _channel(*this, binary, masking, std::forward<Args>(args)...)
        {
        }
        virtual ~HWebSocketServerType() = default;
