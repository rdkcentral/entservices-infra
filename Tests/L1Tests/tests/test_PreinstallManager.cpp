...
namespace WPEFramework
{
    ...
    bool PreinstallManagerImplementation::readPreinstallDirectory(std::list<PackageInfo> &packages)
    {
        ...
        // Clear errno before opendir call
        errno = 0;
        DIR *dir = opendir(preinstallDir.c_str());
        if (!dir)
        {
            int saved_errno = errno;
            LOGINFO("Failed to open directory: %s, errno: %d (%s)", preinstallDir.c_str(), saved_errno, strerror(saved_errno));
            return false;
        }
        ...
        {
            // Extra safety check for null entry
            if (entry == nullptr || entry->d_name == nullptr) {
                LOGWARN("Encountered null directory entry, skipping");
                continue;
            }
            
            std::string filename(entry->d_name);

            // Skip "." and ".." and any empty names
        ...
        if (dir != nullptr) {
            ...
            if (close_result != 0) {
                LOGWARN("closedir failed with errno: %d (%s)", errno, strerror(errno));
            }
        }
        return true;
    }
