...
#define MOCK_USB_DEVICE_BUS_NUMBER_3    102
#define MOCK_USB_DEVICE_ADDRESS_3       003
#define MOCK_USB_DEVICE_PORT_3          125
...
protected:
    ...
    libusb_hotplug_callback_fn libUSBHotPlugCbDevice = nullptr;
...
USBMassStorageTest:: USBMassStorageTest():L2TestMocks()
{
        ...
        .WillByDefault(::testing::Invoke(
            [&](libusb_context *ctx, int events, int flags, int vendor_id, int product_id, int dev_class,
                ...
                if (LIBUSB_HOTPLUG_EVENT_DEVICE == events) {
                    libUSBHotPlugCbDevice = cb_fn;
                    *callback_handle = 3;
                }
...
USBMassStorageTest::~USBMassStorageTest()
{
    ...
    .WillByDefault(::testing::Invoke(
        [&](libusb_context *ctx, libusb_hotplug_callback_handle callback_handle) {
            ...
            }else if (callback_handle == 3) {
                libUSBHotPlugCbDevice = nullptr;  // Deregister the "device left" callback
            }
...
void USBMassStorageTest::Mock_SetDeviceDesc(uint8_t bus_number, uint8_t device_address)
{
    ...
    if (device_address == MOCK_USB_DEVICE_ADDRESS_3)
    {
        std::string vendorFileName = "/tmp/block/sdd/device/vendor";
        std::ofstream outVendorStream(vendorFileName);

        if (!outVendorStream) {
            TEST_LOG("Error opening file for writing!");
        }
        outVendorStream << "SanDisk" << std::endl;
        outVendorStream.close();

        std::string modelFileName = "/tmp/block/sdd/device/model";
        std::ofstream outModelStream(modelFileName);

        if (!outModelStream) {
            TEST_LOG("Error opening file for writing!");
        }
        outModelStream << "MyStick" << std::endl;
        outModelStream.close();
    }
...
void USBMassStorageTest::Mock_SetSerialNumberInUSBDevicePath()
{
    ...
    std::string serialNumFileName3 = "/tmp/bus/usb/devices/102-125/serial";
    std::ofstream serialNumOutFile3(serialNumFileName3);

    if (!serialNumOutFile3) {
        TEST_LOG("Error opening file for writing!");
    }
    serialNumOutFile3 << "SDDSERIAL" << std::endl;
    serialNumOutFile3.close();
    ...
    std::string serialNumFileSdd = "/dev/sdd";
    std::ofstream serialNumOutFileSdd(serialNumFileSdd);

    if (!serialNumOutFileSdd) {
        TEST_LOG("Error opening file for writing!");
    }
    ...
    else
    {
        ...
        if (m_controller_usbmassstorage)
        {
            ...
            if (m_usbmassstorageplugin)
            {
                ...
                /* HotPlug Attach Device 1 Verification */
                Mock_SetDeviceDesc(MOCK_USB_DEVICE_BUS_NUMBER_3, MOCK_USB_DEVICE_ADDRESS_3);

                libusb_device dev = {0};
                dev.bus_number = MOCK_USB_DEVICE_BUS_NUMBER_3;
                dev.device_address = MOCK_USB_DEVICE_ADDRESS_3;
                dev.port_number = MOCK_USB_DEVICE_PORT_3;
                ...
                [](libusb_context *ctx, libusb_device ***list) {
                    ...
                    {
                        uint8_t bus_number = MOCK_USB_DEVICE_BUS_NUMBER_3;
                        uint8_t device_address = MOCK_USB_DEVICE_ADDRESS_3;
                        uint8_t port_number = MOCK_USB_DEVICE_PORT_3;
                ...
                const std::string expectedDevicePath = "/dev/sdd";
                const std::string expectedDeviceName = "102/003";
...
//                 libUSBHotPlugCbDeviceAttached(nullptr, &dev, LIBUSB_HOTPLUG_EVENT_DEVICE, 0);
