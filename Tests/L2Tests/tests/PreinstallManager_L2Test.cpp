...
TEST_F(PreinstallManagerTest, RapidMethodCallsTest)
{
    ...
    // Perform rapid StartPreinstall calls
    for (int i = 0; i < 5; i++) {
...
TEST_F(PreinstallManagerTest, ErrorScenarioTest)
{
    ...
    // Test multiple rapid StartPreinstall calls to stress test error paths
    for (int i = 0; i < 10; i++) {
...
TEST_F(PreinstallManagerTest, EdgeCaseComprehensiveTest)
{
    ...
    // Test rapid registration/unregistration with different notifications
    std::vector<std::promise<std::string>> promises(10);
    std::vector<Core::ProxyType<TestNotification>> notifications;
    ...
    CreateMockPackage("App3", "1.5.0");
    CreateMockPackage("InvalidApp", "");
    CreateMockPackage("App1", "1.1.0"); // Same app, different version

    // Test multiple StartPreinstall calls with different parameters
    for (int i = 0; i < 5; i++) {
        Core::hresult result = mPreinstallManagerPlugin->StartPreinstall(i % 2 == 0);
        EXPECT_TRUE(result == Core::ERROR_NONE || result == Core::ERROR_GENERAL);
...
TEST_F(PreinstallManagerTest, MaximizeCoverageTest)
{
    ...
    // Test with maximum number of notifications to stress test notification management
    std::vector<std::promise<std::string>> promises(20);
    std::vector<Core::ProxyType<TestNotification>> notifications;
    ...
    // Test multiple rapid StartPreinstall calls to exercise different paths
    for (int attempt = 0; attempt < 15; attempt++) {
        bool forceInstall = (attempt % 3 == 0); // Vary force install parameter
        ...
        // Vary the delay to create different timing conditions
        if (attempt % 5 == 0) {
...
TEST_F(PreinstallManagerTest, PackageManagerErrorScenariosTest)
{
    ...
    // Test StartPreinstall multiple times in rapid succession to test error handling
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Very short delay to create potential race conditions
